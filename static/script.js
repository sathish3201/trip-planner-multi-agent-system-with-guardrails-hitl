...
function updateStreamProgress(statusMessage) {
  const reasoning = document.getElementById("supervisorReasoning");
  const section = document.getElementById("workflowSection");
  if (reasoning) {
    reasoning.textContent = statusMessage;
    section.classList.remove("hidden");
  }
}
...
/**
 * Sends request via Server-Sent Events (SSE) for Real-Time Streaming Updates
 */
async function sendMessage() {
  ...
  updateStreamProgress("Initializing multi-agent workflow...");

  const encodedMessage = encodeURIComponent(message);
  let streamUrl = `/api/travel/stream?message=${encodedMessage}`;
  
  if (currentThreadId) {
    streamUrl += `&thread_id=${encodeURIComponent(currentThreadId)}`;
  }
  ...
  eventSource.onmessage = (event) => {
    try {
      ...
      if (data.type === "status") {
        updateStreamProgress(data.content);
      } else if (data.type === "workflow") {
        showWorkflow(data);
      } else if (data.type === "token") {
        latestAnswerMarkdown += data.content;
        const resultBox = document.getElementById("resultBox");
        renderMarkdown(resultBox, latestAnswerMarkdown);
        ...
        if (payload.requires_approval) {
          showResult(payload.itinerary || payload.answer, payload.thread_id, true);
          showApproval(payload);
        } else {
          hideApproval();
          showResult(payload.answer, payload.thread_id, false);
        }

        setLoading(false, "draft");
        eventSource.close();
      } else if (data.type === "error") {
        throw new Error(data.error || "Streaming error encountered.");
      }
    } catch (err) {
  ...
  eventSource.onerror = (err) => {
    ...
    showError("Connection lost or server error occurred during streaming.");
    setLoading(false, "draft");
    eventSource.close();
  };
}
